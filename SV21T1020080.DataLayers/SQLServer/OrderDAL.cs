...
namespace SV21T1020080.DataLayers.SQLServer
{
    public class OrderDAL : BaseDAL, IOrderDAL
    {
        ...
        public int Add(Order data)
        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                var parameters = new
                {
                    ...
                    Status = SV21T1020080.DomainModels.Constants.ORDER_INIT, 
                };
        ...
        public Order? Get(int orderID)
        {
            ...
            using(var connection = OpenConnection())
            {
                var sql = @"select o.*,c.CustomerName,c.ContactName as CustomerContactName,
		                                ...
		                                s.ShipperName,
		                                s.Phone as ShipperPhone,e.FullName as EmployeeName
                            from Orders as o
                                    left join Customers as c on o.CustomerID = c.CustomerID
                                    left join Employees as e on o.EmployeeID = e.EmployeeID
                                    left join Shippers as s on o.ShipperID = s.ShipperID
        ...
        public List<OrderHistory>? ListOfOrderHistory(int customerID, int status)
        {
            List<OrderHistory> list = new List<OrderHistory>();
            using(var connection = OpenConnection())
            {
                var sql = @"select Products.Photo, Products.Unit,Products.ProductName,Products.Price,Orders.Status,orderdetails.quantity,OrderTime,AcceptTime,ShippedTime,FinishedTime
                ...
                list = connection.Query<OrderHistory>(sql:sql, param: parameters, commandType: CommandType.Text).ToList();
                connection.Close();
            }
            return list;
        }
